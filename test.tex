...
\[ V(q) \approx    -\log(\pi(q)) + M \sum_{i \in S} \log(\pi(q|data_i)) \] Data subsampling and omission of the Metropolis acceptance step are two key features of the stochastic gradient MCMC literature. In an overivew the stochastic gradient MCMC literature 
...
\end{align*}
We note that the Cauchy distribution is equivalent to Student-t distribution of degree 1. A thick-tailed prior like the Student-t is known to cause divergent problems. Therefore, it is often written as a Gaussian-Inverse Gamma mixture to reduce posterior correlation. That is, $x \sim t_{\nu}(0,1) $ is 
equivalent to 
\begin{align*}
...
Suppose $W_{i\ell}$ are the weights connecting into the $i$th hidden unit in the $\ell$'th layer of a NN. For example, if there are $n$ incoming units connecting to $m$ hidden units and if the prior in question is Gaussian Inv-Gamma,then we have 
\begin{align*}
...
In the first experiment we compare the Bayesian framework to an early stopping committee of neural networks, a frequentist framework observed to serve as a good baseline. Early stopping is an optimization technique whereby training is stopped when the predictive accuracy on a holdout validate set ceases to improve. A group of neural networks initialized with different random seeds, after training by early stopping,  would then combine their predictions by simple averaging. 

\begin{table}[]
\centering
\begin{tabular}{@{}rlllll@{}}
\multicolumn{1}{l}{}                            & \multicolumn{5}{c}{Test error} \\ \midrule
Method\textbackslash{}Number of ensemble points & 2    & 5   & 50  & 1000 & 2000 \\ \midrule
Early Stopping Committee of NNs                 & 0.1  & 0.5 & 0.5 & 0.13 & 0.3  \\ \midrule
BNN                                             & \multicolumn{5}{c}{0.2}        \\ \bottomrule
\end{tabular}
\caption{Compare the predictive accuracy of the Bayesian Neural Network with Early Stopping Committees of different sizes.}
\label{my-label}
\end{table}
...
In the following experiment we compare the generalized NUTS termination criterion and the exhaustive termination criterion (XHMC). We found that XHMC is capable of generating as efficiently as GNUTS. 
...
Next, we compare Gibbs sampling with joint sampling for a neural network with a Gaussian Inv-Gamma prior put on the input-hidden weights. A standard normal prior is put on the hidden-out weights. More specifically, we assume 
\[W_{ij} \sim \mathcal{N}(0,\sigma^2), \sigma^{-2} \sim \text{Gamma}(0.5,0.5), \]

which is equivalent to a student-t prior of degree 1 on $W_{ij}$. Both centered (CP) and non-centered (NCP) parameterizations are tried for joint sampling. 

Focusing only on the variance parameter $\sigma^2$, we see that the CP encounters many divergent transitions during sampling and results in a too low ESS for meaningful inference. In comparison the NCP gives no divergent transitions and has a reasonable ESS relative to the total number of samples. Compared to the  estimates generated by Gibbs sampling, joint sampling estimates have slightly lower effective sample size, but it does not seem to affect predictive accuracy. 

\begin{table}[]
\centering
\begin{tabular}{@{}lllll@{}}
\toprule
              & test error & sigma2 ess & Min ESS & Median ESS \\ \midrule
Gibbs sampler & 88.4        & 4.38         & 1.29      & 45.3       \\ \midrule
Joint- CP     & 25.8      & 4.86       & 4.69     & 1037         \\ \midrule
Joint - NCP   & 28.4      & 504         & 434     & 1333        \\ \bottomrule
\end{tabular}
\caption{Gibbs vs Joint}
\label{my-label}
\end{table}

\section{SGHMC}
The SGHMC has several extra tuning parameters other than the step size $\epsilon$ and number of leapfrogs $L$ that it inherits from the original HMC sampler. To reduce the number of parameters to tune, we fix these parameters (list them here) at the default values used in the experiments though they are not sufficiently motivated. The parameters remain to be tuned are these. We found that the SGHMC can perform similarly to the HMC although it requires having the right tuning parameter. Because the Metropolis acceptance step is omitted, there is no mechanism to avoid the chain from diverging into a low probability zone in the parameter space. 
...
\multicolumn{5}{c}{Test error}                  \\ \midrule
     & eta = 0.1 & eta = 1 & eta = 0.5 & eta =3 \\ \midrule
L=1  & 0.1       & 0.2     & 0.12      & 0.2    \\ \midrule
L=2  & 0.21      & 0.12    & 0.213     & 0.123  \\ \midrule
L =3 & 0.24      & 0.123   & 0.24      & 0.21   \\ \midrule
L=4  & 0.42      & 0.4     & 0.21      & 0.21   \\ \bottomrule
\end{tabular}
\caption{My caption ep = 2}
\end{minipage}
\begin{minipage}{0.5\linewidth}
\centering
\begin{tabular}{@{}lllll@{}}
...
L=4  & 0.42      & 0.4     & 0.21      & 0.21   \\ \bottomrule
\end{tabular}
\caption{My caption ep = 4}
\end{minipage}
\end{table}


\section{Scaled Priors}
In this experiment, we compared the two normal priors with fixed variances. One is fixed at 1, the other is set to $\frac{1}{N_{\text{hidden}}}$. While we found no difference in their ESS's, the predictive accuracy from the standard normal prior is clearly better. 
...
\toprule
            & Test error & min ess & median ess \\ \midrule
num unit 14 & 0,1        & 0.2     & 0212       \\ \midrule
num unit 20 & 0,2        & 22      & 22         \\ \midrule
num unit 40 & 0,2        & 12      & 124        \\ \bottomrule
\end{tabular}
\caption{My caption unscaled}
\end{minipage}
\end{table}

\section{Covariance Adaptation}
Here we explore whether its necessary or indeed useful to condition the HMC by the full covariance matrix of the target distribution. We experimented with a number of different priors and found that not only is adapting the full matrix more time consuming, it also slows sampling to a halt and destroys any hope of meaningful inference. We attribute this result to the singular nature of neural network models.
...
normal               & 0,1 (3)    & 0.2 (3) & 0212 (3)   & 12 (3)       \\ \midrule
normal inv gamma     & 0,2 (0.3)  & 22 (3)  & 22 (3)     & 12 (3)       \\ \midrule
normal inv gamma ard & 0,2 (0.3)  & 12 (3)  & 124 (3)    & 12 (2)       \\ \bottomrule
\end{tabular}
\caption{My caption}
\label{my-label}
\end{table}
...
Now we fix the prior to be a standard normal distribution and vary the number of layers. We found no evidence of increasing sampling difficulty with increased number of layers. 
...
The following two experiments compare single and double precision for HMC sampling on this neural network model. The first experiment looks at the stability of the leapfrog trajectory. First, a number of points from the posterior distribution are collected by a sampler in double precision, they are each augmented by an independent momentum drawn from a standard normal distribution. Then the list of $(q_i,p_i)$ points are cast into single precision. The two sets of points are then integrated forward for a fixed number of iterations with a reasonably conservative step size and we compare the difference in Hamiltonian between the start and end point of the trajectory.

The second experiment looks at the difference in test error. 

In both experiments we don't find a significant difference between the two, suggesting that the transition to single precision should not harm predictive accuracy and correctness. 
...
Normal               & 0,1   & 12                         \\ \midrule
normal inv gamma     & 0,12  & 12                         \\ \bottomrule
\end{tabular}
\caption{stability}
\end{minipage}
\begin{minipage}{0.5\linewidth}
...
In the following experiment ran a static HMC sampler over a grid of tuning parameter values, and compare the samples generated by the best combination with those generated by a dynamic HMC (NUTS). 
...
Next, we would test the WAIC's ability to select the number of hidden units in a single hidden-layer neural network model. The true test error is estimated by a holdout test set. Different numbers of hidden units are tested and their WAIC computed by dynamic HMC. We found that the model with the lowest WAIC does not match those selected by holdout test error. 

The failure of the WAIC to select the best model might be explained by the poor mixing of the importance sampler when estimating the WAIC, which was noted in a review of predictive checks for Bayesian inference \cite{vehtari2017practical} . 
...
Num Unit= 35    & $3.05 \times 10^4$  & 23.4    & 358 & 1348   \\ \midrule
Num Units = 50  & $4.35 \times 10^4$   & 20.4  & 714  & 1342                \\ \midrule
Num Units = 75 &$ 6.36 \times 10^4$   & 21.2  & 883  & 1351                 \\ \midrule
Num Units = 90 &$ 7.57 \times 10^4$  & 21.2   & 562  & 1359                 \\ \bottomrule
\end{tabular}
\caption{WAIC}
\label{my-label}
\end{table}
...
&c^2 \sim \text{Inv-gamma}(2,8).\\
\end{align*}
We found that while the regularized horseshoe prior performs better than the original horseshoe prior in terms of having zero divergent transitions, none of the horseshoe priors can be said to induce great sampling efficiency, as both result in models whose posterior distribution are difficult to sample, resulting in low ESS. What's more is that even the Normal Inv-Gamma prior does not give better predictive performance than the standard normal prior, which we have already seen in earlier experiments comparing Gibbs sampling to joint sampling for the hyperparameter. 
\begin{table}[]
\centering
