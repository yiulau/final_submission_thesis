...
\begin{algorithm}
    ...
    $ w = w + w'$\;
    $ s = s \mathbb{I}[(q_+ - q_-) ^T p_- \ge 0 ] \mathbb{I}[(q_+ - q_-) ^T p_+ \ge 0 ] $\;
    $j = j+1 $ \;
...
\KwRet{$q',p',q',p',q',s',w',p_{s'}$} \;
	...
	\If{$s'=\text{True}$}{
		...
		$p_+^\# \leftarrow M^{-1} p_{+} $\;
		$p_-^\# \leftarrow M^{-1} p_{-} $\;
		$ s' \leftarrow s'' \mathbb{I}[(q_+ - q_- )^T p_-^\# \ge 0 ] \mathbb{I}[(q_+ - q_- )^T p_+^\# \ge 0 ]$ \;
		$w' \leftarrow w' + w'' $ \;
		$p_{s} = p_{s} + p_{s''} $\;
...
The step size $\epsilon$ is arguably the most important tuning parameter for the HMC. Too large of a step size would cause the leapfrog trajectories to diverge, although the divergence threshold is generally model dependent. For the 1-dimensional potential energy function $V(q) = \frac{q^2}{2\sigma^2}$,
