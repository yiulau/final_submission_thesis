...
model our uncertainty about it with a prior function $\pi(q)$. Upon observing the data, we can perform Bayesian inference by using information from the posterior distribution 
...
Suppose we have a an unnormalized density $\pi(q)$, and $h(q,p)$ is a proposal function such that conditional on the current state $q$, the probability of moving from $q$ to a measurable set $A$ is denoted by $h(x,A)$, we can calculate the Hastings ratio
...
\For{$i = 1:n$}{
  	Draw $q_{prop}$ from $h(q_{i-1},\cdot)$ \;  	
  	Accept $q_{prop}$ with probability $\min(1,r(q_{prop},q_{i-1}))$ \;
