...
\begin{algorithm}
    ...
    \KwIn{initial position $\theta^0$, step size $\epsilon$, joint density $\pi$} 
    Re-sample $r^0 \sim \mathcal{N}(0,\mathbb{I})$ \;
    Initialize $\theta^{-} = \theta^0 , \theta^{+} = \theta^0 , r^{-} =r^0 , r^+ = r^0, r^+_{\#} = M^{-1}p^+,r^-_{\#} = M^{-1}p^-, p_s = p_0 j =0 ,	\theta^{next} = \theta^0 , s = 1 , w = \pi(\theta^0,r^0)$ \;
    \While{$s=1$}{
    Choose a direction $v_j \sim \text{Uniform}(-1,1) $ \;
    \eIf{$v_j = -1$}{
    		$\theta^-,r^-,-,-,\theta',s',w',p_{s'} \leftarrow \text{G-BuildTree}(\theta^-,r^-,v_j,j,\epsilon)$\;
    }{
    		$-,-,\theta^+,r^+,\theta',s',w',p_{s'} \leftarrow \text{G-BuildTree}(\theta^+,r^+,v_j,j,\epsilon)$\;
    }
    \If{$s'=\text{True}$}{
    ...
    $ w = w + w'$\;
    $ s = s' \mathbb{I}[(\theta^+ - \theta^-) \cdot p^\sharp_{-} \ge 0 ] \mathbb{I}[(\theta^+ - \theta^-) \cdot p^\sharp_{+}  \ge 0 ] $\;
    $j = j+1 $ \;
...
$ w' \leftarrow \pi(\theta',r') $ \;
$s' = \text{True}$ \;
\KwRet{$\theta',r',\theta',r',\theta',s',w',p_{s'}$} \;

}{
	$p_s = 0 $\;
	$\theta^-,r^-,\theta^+,r^+,\theta',s',w',p_{s'} \leftarrow \text{G-BuildTree}(\theta,r,v,j-1,\epsilon) $ \;
	
	\If{$s'=\text{True}$}{
		$p_s = p_s + p_{s'}$\;
		\eIf{$v=-1$}{
			$\theta^-,r^-,-,-,\theta'',s'',w'',p_{s''} \leftarrow \text{G-BuildTree}(\theta^-,r^-,v,j-1,\epsilon)$ \;
		}{
			$-,-,\theta^+,r^+,\theta'',s'',w'',p_{s''} \leftarrow \text{G-BuildTree}(\theta^+,r^+,v,j-1,\epsilon)$ \;
		}
		\If{$s''=\text{True}$}{
		...
		$p^\sharp_{+} \leftarrow M^{-1} p_{+} $\;
		$p^\sharp_{-} \leftarrow M^{-1} p_{-} $\;
		$ s' \leftarrow s'' \mathbb{I}[(\theta^+ - \theta^- )\cdot p^\sharp_{-} \ge 0 ] \mathbb{I}[(\theta^+ - \theta^- )\cdot p^\sharp_{+} \ge 0 ]$ \;
		$w' \leftarrow w' + w'' $ \;
		$p_{s} = p_{s} + p_{s''} $\;
...
\begin{algorithm}
    ...
    \KwIn{initial position $\theta^0$, step size $\epsilon$, joint density $\pi$} 
    Re-sample $r^0 \sim \mathcal{N}(0,\mathbb{I})$ \;
    Initialize $\theta^{-} = \theta^0 , \theta^{+} = \theta^0 , r^{-1} =r^0 , r^+ = r^0, j =0 ,	\theta^{next} = \theta^0 , s = 1 , w = \pi(\theta^0,r^0), \text{ave} = \frac{dG}{dt}(q^0,p^0)$ \;
    \While{$s=1$}{
    Choose a direction $v_j \sim \text{Uniform}(-1,1) $ \;
    \eIf{$v_j = -1$}{
    		$\theta^-,r^-,-,-,\theta',s',w',\text{ave}' \leftarrow \text{XHMC-BuildTree}(\theta^-,r^-,v_j,j,\epsilon)$\;
    }{
    		$-,-,\theta^+,r^+,\theta',s',w',\text{ave}' \leftarrow \text{XHMC-BuildTree}(\theta^+,r^+,v_j,j,\epsilon)$\;
    }
    \If{$s'=1$}{
...
\KwRet{$\theta^-,r^-,\theta^+,r^+,\theta',s',w',\text{ave}'$} \;

}{
	$\theta^-,r^-,\theta^+,r^+,\theta',s',w',\text{ave}' \leftarrow \text{XHMC-BuildTree}(\theta,r,v,j-1,\epsilon) $ \;
	
	\If{$s'=1$}{
		\eIf{$v=-1$}{
			$\theta^-,r^-,-,-,\theta",s'',w'',\text{ave}'' \leftarrow \text{XHMC-BuildTree}(\theta^-,r^-,v,j-1,\epsilon)$ \;
		}{
			$-,-,\theta^+,r^+,\theta'',s'',w'',\text{ave}'' \leftarrow \text{XHMC-BuildTree}(\theta^+,r^+,v,j-1,\epsilon)$ \;
		}
		With probability $\min(1, \frac{w''}{w'+w''})$ , set $\theta' = \theta''$. \;
	...
	\KwRet{$\theta^-,r^-,\theta^+,r^+,\theta',s',w',{ave}'$ }\;
}
    }
